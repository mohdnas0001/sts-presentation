...
const pains = [
  ...
  'Manual and delayed reporting cycles',
  'Inconsistent KPI definitions',
  'Weak early-risk signaling',
  'Limited cross-pillar visibility',
]
...
const impacts = [
  ...
  'Lower policy impact traceability',
]
  ...
  return (
    <motion.div custom={direction} variants={variants} initial="enter" animate="center" exit="exit"
      ...
      <motion.h2 initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}
        className="text-5xl font-bold text-white mb-8">Current <span className="text-red-400">Monitoring Gaps</span></motion.h2>
      ...
      <div className="grid grid-cols-5 gap-6 max-w-5xl w-full">
        ...
        <motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}
          className="col-span-3 bg-[#112240] rounded-2xl p-6 flex flex-col gap-4">
          <h3 className="text-white font-semibold text-base mb-2">Pain Points</h3>
          <ul className="space-y-3">
            {pains.map((p, i) => (
              <motion.li key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 + i * 0.08 }}
                className="flex items-start gap-3 text-white/70 text-sm">
                <span className="text-red-400 text-lg mt-0.5">●</span>{p}
              </motion.li>
            ))}
          </ul>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }}
            className="mt-2 bg-red-900/20 border border-red-500/30 rounded-xl p-4">
            <div className="flex items-center gap-2 mb-3">
              <FaExclamationTriangle className="text-red-400 text-base" />
              <p className="text-red-400 font-semibold text-sm">Cumulative Impact</p>
            </div>
            {impacts.map((t, i) => (
              <div key={i} className="flex items-center gap-2 mb-1.5">
                <span className="text-red-400 text-xs">⚠</span>
                <p className="text-red-300/80 text-xs">{t}</p>
              </div>
            ))}
          </motion.div>
        </motion.div>

        {/* Right 40% — Radar chart */}
        <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}
          className="col-span-2 bg-[#112240] rounded-2xl p-4 flex flex-col">
          <h3 className="text-white font-semibold text-sm text-center mb-1">Current State vs Desired State</h3>
          <ResponsiveContainer width="100%" height={300}>
            <RadarChart data={radarData}>
              <PolarGrid stroke="#ffffff15" />
              <PolarAngleAxis dataKey="subject" tick={{ fill: '#ffffff70', fontSize: 10 }} />
