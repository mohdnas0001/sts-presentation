  ...
  return (
    <motion.div custom={direction} variants={variants} initial="enter" animate="center" exit="exit"
      ...
      <motion.h2 initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}
        ...
        Introducing the <span className="text-[#00A859]">Smart Tracking System (STS)</span>
      </motion.h2>
      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
        className="text-white/60 text-center max-w-2xl mb-10 text-sm leading-relaxed">
        A centralized digital platform to monitor, evaluate, and report all SRAP 2.0 initiatives in real time.
      </motion.p>

      <div className="relative w-80 h-80">
        {/* Outer ring */}
        <div className="absolute inset-0 rounded-full border border-[#00A859]/10 animate-pulse" />

        {/* Center badge */}
        <div className="absolute inset-0 flex items-center justify-center z-10">
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.3, type: 'spring' }}
            className="w-24 h-24 rounded-full bg-gradient-to-br from-[#00A859] to-green-700 flex items-center justify-center shadow-2xl shadow-green-500/40">
            <div className="text-center">
              <p className="text-white font-bold text-xl leading-none">STS</p>
              <p className="text-green-200 text-xs mt-1">Platform</p>
            </div>
          </motion.div>
        </div>
